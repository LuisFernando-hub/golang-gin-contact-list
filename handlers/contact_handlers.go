...
func CreateContacts(c *gin.Context) {
	...
	if err := config.DB.Create(&contact).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("Failed to fetch create contact: %v", err.Error()),
		})
		return
	}
...
func DeleteContacts(c *gin.Context) {
	...
	if err := config.DB.Delete(&contact).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": fmt.Sprintf("Failed to fetch delete contact with id %v: %v", id, err.Error()),
		})
		return
	}
...
func UpdateContacts(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		"msg":  fmt.Sprintf("Contact with id %v update successfully!", id),
		"data": dbContact,
	})
}
